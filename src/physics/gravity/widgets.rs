...
impl GravityWidget {
    fn from_config(c: &GravityWidgetConfig) -> Self {
        let mode = c
            ...
            .parse::<GravityMode>()
            .ok()
            .unwrap_or(GravityMode::Attract);
        let falloff = c
            .falloff
            .parse::<Falloff>()
            .ok()
            .unwrap_or(Falloff::InverseLinear);
        let r2 = if c.radius > 0.0 {
            c.radius * c.radius
        } else {
            0.0
        };
        Self {
            ...
            falloff,
            enabled: c.enabled,
            physics_collider: c.physics_collider,
            radius2: r2,
        }
    }
}
        ...
        app.register_type::<GravityWidget>()
            ...
            .add_systems(
                ...
                (
                    ...
                    accumulate_widget_forces
                        .after(toggle_widget_on_tap)
                        .in_set(PrePhysicsSet),
                    apply_accumulated_widget_forces
                        .after(accumulate_widget_forces)
                        .in_set(PrePhysicsSet),
...
fn spawn_configured_gravity_widgets(
    ...
    for (i, wc) in widgets.iter().enumerate() {
        ...
        let mesh = meshes.add(Mesh::from(Circle {
            radius: GRAVITY_WIDGET_ICON_RADIUS,
        }));
...
fn primary_pointer_world_pos(
    ...
    if let Some(t) = touches.iter().next() {
        return camera_q
            ...
            .next()?
            .0
            .viewport_to_world_2d(camera_q.iter().next()?.1, t.position())
            .ok();
    }
...
fn toggle_widget_on_tap(
    ...
    if let Some((entity, _)) = best {
        if let Ok((_e, _tf, mut gw, mat_handle)) = q_widgets.get_mut(entity) {
            ...
            if let Some(mat) = materials.get_mut(&mat_handle.0) {
                let base = match gw.mode {
                    GravityMode::Attract => (0.2, 0.4, 0.95),
                    GravityMode::Repulse => (0.95, 0.25, 0.3),
                };
                let alpha = if gw.enabled { 0.85 } else { 0.25 };
                mat.color = Color::srgba(base.0, base.1, base.2, alpha);
            }
            ew.write(WidgetToggled {
                id: gw.id,
                enabled: gw.enabled,
            });
...
fn accumulate_widget_forces(
    ...
    for (w_tf, gw) in q_widgets.iter() {
        ...
        for (ball_e, b_tf) in q_balls.iter_mut() {
            ...
            if gw.mode == GravityMode::Repulse {
                dir = -dir;
            }
            if gw.radius2 > 0.0 && dist2 > gw.radius2 {
                continue;
            }
            let dist = dist2.sqrt();
            ...
            let scalar = match gw.falloff {
                ...
                Falloff::SmoothEdge => {
                    if gw.radius > 0.0 {
                        let t = (1.0 - dist / gw.radius).clamp(0.0, 1.0);
                        base * (t * t * (3.0 - 2.0 * t))
                    } else {
                        base
                    }
...
fn apply_accumulated_widget_forces(
    ...
    for (e, f) in acc.0.drain(..) {
        *summed.entry(e).or_insert(Vec2::ZERO) += f;
    }
    for (e, mut accel) in summed.into_iter() {
        if accel.length() > GRAVITY_WIDGET_MAX_ACCEL {
            accel = accel.normalize() * GRAVITY_WIDGET_MAX_ACCEL;
        }
        if let Ok(mut vel) = q_vel.get_mut(e) {
            vel.linvel += accel * dt;
        }
        if let Ok(mut ef) = q_force.get_mut(e) {
            ef.force = accel;
        } else {
...
mod tests {
    ...
    fn attract_vs_repulse_direction() {
        ...
        assert!(
            acc.0.len() == 2,
            "expected 2 force entries (one per widget) got {}",
            acc.0.len()
        );
        let mut attract_force = None;
        let mut repulse_force = None;
        for (_e, f) in acc.0.iter() {
            if f.x > 0.0 {
                attract_force = Some(f.x);
            } else if f.x < 0.0 {
                repulse_force = Some(f.x);
            }
        }
        assert!(
            attract_force.is_some() && repulse_force.is_some(),
            "forces not found: {:?}",
            acc.0
        );
    }
