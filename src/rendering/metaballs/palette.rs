...
pub fn ensure_palette_capacity(
    ...
    let mut new_cap = if storage.capacity == 0 {
        PALETTE_INITIAL_CAPACITY
    } else {
        storage.capacity
    };
    while new_cap < needed {
        new_cap = (new_cap * 2).min(PALETTE_MAX_CAPACITY);
        if new_cap == PALETTE_MAX_CAPACITY {
            break;
        }
    }
    if needed > new_cap {
    ...
    if let Some(h) = &storage.handle {
        if buffers.get(h).is_some() {
            if let Some(mut_ref) = buffers.get_mut(h) {
                *mut_ref = ssb;
            } else {
                let new_h = buffers.add(ssb);
                storage.handle = Some(new_h);
            }
        } else {
            let new_h = buffers.add(ssb);
            storage.handle = Some(new_h);
...
pub fn build_cpu_palette(
    clusters: &crate::physics::clustering::cluster::Clusters,
) -> ClusterPaletteCpu {
    ...
    for (id, ci, col) in triples.into_iter() {
        out.ids.push(id);
        out.color_indices.push(ci);
        out.colors.push(col);
    }
    out
}
...
mod tests {
    ...
    fn ordering_deterministic_by_id() {
        ...
        assert_eq!(
            palette1.ids, palette2.ids,
            "Sorted ids must be stable across frames"
        );
