...
#[derive(Resource, Debug, Clone, Copy, PartialEq, Eq)] pub enum MetaballRenderMode { Classic, Bevel } impl MetaballRenderMode { pub const ALL: [Self;2] = [Self::Classic, Self::Bevel]; }
...
#[derive(Component)] pub struct MetaballsQuad; #[derive(Component)] pub struct MetaballsBevelQuad; // second quad for bevel style
...
pub struct MetaballsPlugin; impl Plugin for MetaballsPlugin { fn build(&self, app: &mut App) {
        ...
        app.init_resource::<MetaballsToggle>()
            ...
            .add_plugins((Material2dPlugin::<MetaballsMaterial>::default(), Material2dPlugin::<MetaballsBevelMaterial>::default(),))
            .add_systems(Startup, (initialize_toggle_from_config, apply_config_to_params, setup_metaballs))
            .add_systems(Update, (update_metaballs_material, update_metaballs_bevel_material, cycle_metaball_mode, apply_metaball_mode, resize_fullscreen_quad, tweak_metaballs_params)); } }
...
fn setup_metaballs(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>, mut materials: ResMut<Assets<MetaballsMaterial>>, mut bevel_mats: ResMut<Assets<MetaballsBevelMaterial>>, windows: Query<&Window>) {
    ...
    // Spawn both, bevel hidden initially
    ...
    commands.spawn((Mesh2d::from(mesh_handle), MeshMaterial2d(bevel_handle), Transform::from_xyz(0.0, 0.0, 50.0), Visibility::Hidden, MetaballsBevelQuad));
}
...
fn apply_metaball_mode(mode: Res<MetaballMode>, mut q_classic: Query<&mut Visibility, (With<MetaballsQuad>, Without<MetaballsBevelQuad>)>, mut q_bevel: Query<&mut Visibility, (With<MetaballsBevelQuad>, Without<MetaballsQuad>)>, mut q_bg: Query<&mut Visibility, (With<crate::rendering::background::background::BackgroundQuad>, Without<MetaballsQuad>, Without<MetaballsBevelQuad>)>) {
    ...
    match current {
        MetaballRenderMode::Classic => {
            *vis_classic = Visibility::Visible;
            *vis_bevel = Visibility::Hidden;
            for mut v in q_bg.iter_mut() { *v = Visibility::Visible; }
        }
        MetaballRenderMode::Bevel => {
            *vis_classic = Visibility::Hidden;
            *vis_bevel = Visibility::Visible;
            for mut v in q_bg.iter_mut() { *v = Visibility::Hidden; }
        }
    }
}
