...
impl Plugin for InputActionsPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<InputMap>()
            ...
            .add_systems(
                PreUpdate,
                (system_collect_inputs, system_evaluate_bindings)
                    .chain()
                    .in_set(InputActionUpdateSet),
            );
    }
}
...
fn load_initial_input_map(mut commands: Commands) {
    ...
    let raw: String = include_str!("../../../assets/config/input.toml").to_string();
    ...
    if !parsed.errors.is_empty() {
        for e in parsed.errors {
            error!("INPUT MAP ERROR: {e}");
        }
    } else {
        info!(
            "Input map loaded: {} actions",
            parsed.input_map.actions.len()
        );
    }
