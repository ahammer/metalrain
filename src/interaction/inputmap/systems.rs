...
pub fn system_collect_inputs(
    ...
    let mut wheel_v = 0.0f32; let mut wheel_h = 0.0f32; for ev in wheel_evr.read() { match ev.unit { bevy::input::mouse::MouseScrollUnit::Line => { wheel_v += ev.y; wheel_h += ev.x; }, bevy::input::mouse::MouseScrollUnit::Pixel => { wheel_v += ev.y * 0.1; wheel_h += ev.x * 0.1; } } }
    ...
    if !input_map.virtual_axes.is_empty() { let snapshot = input_map.virtual_axes.clone(); let mut vals = vec![0.0; snapshot.len()]; for (i, va) in snapshot.iter().enumerate() { let mut v = 0.0; match va.pos { RawBindingToken::Key(k) => if keyboard.pressed(k) { v += 1.0; }, RawBindingToken::MouseBtn(mb) => if mouse_buttons.pressed(mb) { v += 1.0; }, RawBindingToken::MouseMoveX => { if input_map.mouse_delta.x > 0.0 { v += (input_map.mouse_delta.x/10.0).clamp(0.0,1.0); } }, RawBindingToken::MouseMoveY => { if input_map.mouse_delta.y > 0.0 { v += (input_map.mouse_delta.y/10.0).clamp(0.0,1.0); } }, RawBindingToken::WheelVert => { if input_map.wheel_delta.y > 0.0 { v += (input_map.wheel_delta.y/10.0).clamp(0.0,1.0); } }, RawBindingToken::WheelHorz => { if input_map.wheel_delta.x > 0.0 { v += (input_map.wheel_delta.x/10.0).clamp(0.0,1.0); } }, _=>{} } match va.neg { RawBindingToken::Key(k) => if keyboard.pressed(k) { v -= 1.0; }, RawBindingToken::MouseBtn(mb) => if mouse_buttons.pressed(mb) { v -= 1.0; }, RawBindingToken::MouseMoveX => { if input_map.mouse_delta.x < 0.0 { v -= (-input_map.mouse_delta.x/10.0).clamp(0.0,1.0); } }, RawBindingToken::MouseMoveY => { if input_map.mouse_delta.y < 0.0 { v -= (-input_map.mouse_delta.y/10.0).clamp(0.0,1.0); } }, RawBindingToken::WheelVert => { if input_map.wheel_delta.y < 0.0 { v -= (-input_map.wheel_delta.y/10.0).clamp(0.0,1.0); } }, RawBindingToken::WheelHorz => { if input_map.wheel_delta.x < 0.0 { v -= (-input_map.wheel_delta.x/10.0).clamp(0.0,1.0); } }, _=>{} } vals[i] = (v * va.scale).clamp(-1.0,1.0); } input_map.virtual_axis_values = vals; }
}
...
pub fn system_evaluate_bindings(
    ...
    for binding in bindings_snapshot { let mut all_active = true; for token in &binding.tokens { let active = match token { RawBindingToken::Key(k) => keyboard.pressed(*k), RawBindingToken::MouseBtn(b) => mouse_buttons.pressed(*b), RawBindingToken::TouchTap => { // will be set via gesture injection, treat here as false for chord logic
                false
            }, RawBindingToken::TouchDrag => input_map.gesture_rt.dragging, RawBindingToken::MouseMoveX => input_map.mouse_delta.x.abs() > 0.0, RawBindingToken::MouseMoveY => input_map.mouse_delta.y.abs() > 0.0, RawBindingToken::WheelVert => input_map.wheel_delta.y.abs() > 0.0, RawBindingToken::WheelHorz => input_map.wheel_delta.x.abs() > 0.0 }; if !active { all_active = false; break; } } let rt = &mut input_map.bindings_runtime[binding.id as usize]; if all_active { if !rt.active { rt.active = true; rt.just_pressed = true; rt.hold_elapsed = 0.0; } else { rt.hold_elapsed += dt; } } else if rt.active { rt.active = false; rt.just_released = true; } }
