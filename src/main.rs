...
fn load_config() -> GameConfig {
    ...
    for e in errors { warn!("CONFIG LOAD ISSUE: {e}"); }
    if used.is_empty() { warn!("No config files loaded; using defaults"); } else { info!(?used, "Config layers loaded"); }
    cfg
}
...
fn main() {
    ...
    for warn in cfg.validate() { warn!("CONFIG WARNING: {warn}"); }
    let mut app = App::new();
    app.insert_resource(cfg.clone())
        .add_plugins(DefaultPlugins.set(WindowPlugin { primary_window: Some(Window { title: cfg.window.title.clone(), resolution: (cfg.window.width, cfg.window.height).into(), resizable: true, ..default() }), ..default() }));
    app.add_plugins(GamePlugin);
    #[cfg(feature = "debug")]
    { app.add_plugins(RapierDebugRenderPlugin::default()); }
    #[cfg(not(feature = "debug"))]
    { if cfg.rapier_debug { app.add_plugins(RapierDebugRenderPlugin::default()); } }
    #[cfg(target_arch = "wasm32")] {
