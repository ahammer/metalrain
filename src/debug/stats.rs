...
pub fn debug_stats_collect_system(time: Res<Time>, mut state: ResMut<DebugState>, mut stats: ResMut<DebugStats>, q_balls: Query<&Ball>, clusters: Res<Clusters>) {
    ...
    let inst_fps = 1.0 / dt;
    if stats.fps == 0.0 { stats.fps = inst_fps; } else { stats.fps = stats.fps * 0.9 + inst_fps * 0.1; }
    let inst_ms = dt * 1000.0;
    if stats.frame_time_ms == 0.0 { stats.frame_time_ms = inst_ms; } else { stats.frame_time_ms = stats.frame_time_ms * 0.9 + inst_ms * 0.1; }
