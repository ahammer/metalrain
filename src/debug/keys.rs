...
pub fn debug_key_input_system(input_map: Option<Res<InputMap>>, mut state: ResMut<DebugState>) {
    ...
    if input_map.just_pressed("DebugMode3") { new_mode = Some(DebugRenderMode::MetaballHeightfield); }
    if input_map.just_pressed("DebugMode4") { new_mode = Some(DebugRenderMode::MetaballColorInfo); }
    ...
    if input_map.just_pressed("ToggleOverlay") { state.overlay_visible = !state.overlay_visible; }
}
