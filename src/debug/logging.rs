...
pub fn debug_logging_system(
    ...
    let baseline_ft_ms = if stats.fps > 0.1 {
        1000.0 / stats.fps
    } else {
        ft_ms
    };
    if state
        .frame_counter
        .saturating_sub(state.last_frame_time_alert_frame)
        > cooldown
    ...
    if state.last_ball_count == 0 {
        ...
        .frame_counter
        .saturating_sub(state.last_ball_alert_frame)
        > cooldown
    {
    ...
    if state.last_cluster_count == 0 {
        ...
        .frame_counter
        .saturating_sub(state.last_cluster_alert_frame)
        > cooldown
    {
