...
metab all={mben} iso {iso:.2} nz {nz:.1} mtl {metal:.2} rough {rough:.2} env {env:.2} spec {spec:.2} hard {hard} cbexp {cbexp:.2} rmul {rmul:.2}",
            ...
            // radius multiplier intentionally omitted previously; add concise token
        );
    }
}
