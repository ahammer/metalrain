        ...
        if let Some((push_vec, normal)) = compute_pair_push(dist, r1.0, r2.0, sep_cfg, delta) {
            ...
            if sep_cfg.velocity_dampen > 0.0 {
                ...
                vel_normals
                    .entry(*e2)
                    .and_modify(|v| *v -= normal)
                    .or_insert(-normal); // opposite direction
            }
        }
    }
...
fn compute_pair_push(
    dist: f32,
    r1: f32,
    r2: f32,
    cfg: &crate::config::CollisionSeparationConfig,
    delta: Vec2,
) -> Option<(Vec2, Vec2)> {
    if dist <= 0.0 {
        return None;
...
mod tests {
    ...
    fn compute_pair_push_basic() {
        ...
        let cfg = crate::config::CollisionSeparationConfig {
            enabled: true,
            overlap_slop: 0.99,
            push_strength: 1.0,
            max_push: 100.0,
