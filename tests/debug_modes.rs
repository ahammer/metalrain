...
use ball_matcher::debug::{DebugRenderMode, DebugState};
...
fn mode_switch_via_keys() {
    ...
    fn mark_pressed(app: &mut App, action: &str) {
        let id_opt = {
            ...
            im.name_to_id.get(action).copied()
        };
        if let Some(id) = id_opt {
            let mut im = app.world_mut().resource_mut::<InputMap>();
            if let Some(ActionDynamicState::Binary(b)) = im.dynamic_states.get_mut(id.0 as usize) {
                b.pressed = true;
                b.just_pressed = true;
            }
        }
    ...
    app.update();
    assert_eq!(
        app.world().resource::<DebugState>().mode,
        DebugRenderMode::RapierWireframe
    );
    ...
    assert_eq!(
        app.world().resource::<DebugState>().mode,
        DebugRenderMode::MetaballHeightfield
    );
    ...
    assert_eq!(
        app.world().resource::<DebugState>().mode,
        DebugRenderMode::MetaballColorInfo
    );
}
