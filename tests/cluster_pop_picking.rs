...
fn base_cluster_pop() -> ClusterPopConfig {
    ClusterPopConfig {
        ...
        impulse: None, outward_bonus: None, despawn_delay: None, fade_duration: None, fade_scale_end: None,
        collider_shrink: None, collider_min_scale: None, velocity_damping: None, spin_jitter: None,
    }
}
...
fn spawn_ball(app: &mut App, pos: Vec2, radius: f32, color_index: usize) -> Entity {
    ...
    app.world_mut().spawn((Ball, BallRadius(radius), BallMaterialIndex(color_index), Transform::from_xyz(pos.x, pos.y, 0.0), GlobalTransform::default(), Velocity::zero())).id()
}

fn force_clusters(app: &mut App) { app.update(); }
...
fn perform_pick(app: &mut App, world_pos: Vec2) -> Option<usize> {
    ...
    let lifecycle_store: Vec<Option<PaddleLifecycle>> = entities.iter().map(|(_,_,r,has)| if *has { Some(PaddleLifecycle { elapsed:0.0, grow_duration:0.0, hold_duration:0.0, shrink_duration:0.0, peak_scale:1.0, freeze_mode: ball_matcher::interaction::cluster_pop::FreezeMode::ZeroVelEachFrame, base_radius:r.0, fade_alpha:false, fade_curve:0, collider_scale_curve:0, alpha_base:0.0 }) } else { None }).collect();
    let iter = entities.iter().zip(lifecycle_store.iter()).map(|(data, plc)| { let (e,t,r,_has)=data; (*e, t, r, plc.as_ref()) });
    if let Some((_ball, ci, _r, _d2)) = pick_ball_cluster(world_pos, &clusters, &index, iter, &cfg.interactions.cluster_pop) { Some(ci) } else { None }
}
...
fn hit_single_ball() {
    ...
    assert!(picked.is_some(), "Expected to pick the single ball's cluster");
}

#[test]
fn no_hit_outside_radius() {
    let mut app = test_app(|cp| { cp.ball_pick_radius = 20.0; });
    spawn_ball(&mut app, Vec2::ZERO, 10.0, 0);
    force_clusters(&mut app);
...
fn overlapping_aabb_regression() {
    ...
    for i in 0..5 { spawn_ball(&mut app, Vec2::new(200.0 + i as f32 * 5.0, 0.0), 12.0, 0); }
    ...
    assert_eq!(clusters.0[picked].color_index, 1, "Should pick smaller nearby cluster, not larger overlapping AABB");
}

#[test]
fn distance_tie_larger_radius_preferred() {
    let mut app = test_app(|cp| { cp.prefer_larger_radius_on_tie = true; cp.ball_pick_radius_scale_with_ball = false; cp.ball_pick_radius = 50.0; });
    ...
    {
        let clusters = app.world().get_resource::<Clusters>().unwrap();
        if clusters.0.len() == 1 { // merged -> rebuild with diff colors
    ...
    assert!(chosen.entities.iter().any(|&e| {
        let br = app.world().get::<BallRadius>(e).unwrap().0; br >= 12.0 - 1e-4
    }), "Expected cluster of larger radius ball");
}

#[test]
fn distance_tie_without_radius_pref_cluster_size_breaker() {
    let mut app = test_app(|cp| { cp.prefer_larger_radius_on_tie = false; cp.ball_pick_radius_scale_with_ball = false; cp.ball_pick_radius = 50.0; });
    ...
    let picked = perform_pick(&mut app, Vec2::new(0.0, 0.0)).expect("must pick a cluster");
    let clusters = app.world().get_resource::<Clusters>().unwrap();
    assert_eq!(clusters.0[picked].color_index, 1, "Expected larger cluster to win tie");
}
