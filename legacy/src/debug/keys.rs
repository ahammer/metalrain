...
pub fn debug_key_input_system(keys: Res<ButtonInput<KeyCode>>, mut state: ResMut<DebugState>) {
    ...
    if keys.just_pressed(KeyCode::Digit2) { new_mode = Some(DebugRenderMode::RapierWireframe); }
    if keys.just_pressed(KeyCode::Digit3) { new_mode = Some(DebugRenderMode::MetaballHeightfield); }
    if keys.just_pressed(KeyCode::Digit4) { new_mode = Some(DebugRenderMode::MetaballColorInfo); }
