...
fn gather_metaballs(
    ...
    query: Query<(&Transform, &MetaBall, Option<&MetaBallColor>, Option<&MetaBallCluster>)>,
    mut logged: Local<LoggedOnce>,
) {
    if !**flag { return; } // no changes -> skip work
    buffer.balls.clear();
...
mod tests {
    ...
    fn setup_app() -> App {
        ...
        app.insert_resource(crate::coordinates::MetaballCoordinateMapper::new(UVec2::new(1024,1024), Vec2::new(-512.0,-512.0), Vec2::new(512.0,512.0)));
    ...
    for i in 0..10 { app.world_mut().spawn((Transform::from_translation(Vec3::new(i as f32, i as f32, 0.0)), MetaBall { radius_world: 5.0 }, MetaBallColor(LinearRgba::new(1.0,1.0,1.0,1.0)))); }
        app.update();
    ...
    fn no_repack_without_changes() {
        let mut app = setup_app();
    app.world_mut().spawn((Transform::from_translation(Vec3::new(0.0,0.0,0.0)), MetaBall { radius_world: 5.0 }));
    ...
    fn repack_after_component_change() {
        let mut app = setup_app();
    let e = app.world_mut().spawn((Transform::from_translation(Vec3::new(0.0,0.0,0.0)), MetaBall { radius_world: 5.0 })).id();
        app.update(); // initial pack
        // mutate component
        {
