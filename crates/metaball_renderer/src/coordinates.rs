...
pub fn screen_to_world(
    ...
    camera
        .viewport_to_world_2d(cam_transform, screen)
        .ok()
        .map(|v| v.extend(0.0))
}
...
pub fn screen_to_metaball_uv(
    ...
    camera: &Camera,
    cam_transform: &GlobalTransform,
    mapper: &MetaballCoordinateMapper,
) -> Option<Vec2> {
...
mod tests {
    ...
    fn corners_map_correctly() {
        ...
        assert_eq!(
            mapper.world_to_metaball(Vec3::new(-256.0, -256.0, 0.0)),
            Vec2::new(0.0, 0.0)
        );
        assert_eq!(
            mapper.world_to_metaball(Vec3::new(256.0, -256.0, 0.0)).x,
            512.0
        );
        assert_eq!(
            mapper.world_to_metaball(Vec3::new(-256.0, 256.0, 0.0)).y,
            512.0
        );
    }
    #[test]
    ...
    fn uv_in_range() {
        ...
        for i in 0..10 {
            ...
            for j in 0..10 {
                let y = -1.0 + 0.2 * j as f32;
                let uv = mapper.metaball_to_uv(mapper.world_to_metaball(Vec3::new(x, y, 0.0)));
                assert!(
