...
#[derive(Resource)]
pub struct GpuNormalsPipeline { pub layout: BindGroupLayout, pub pipeline_id: CachedComputePipelineId }
#[derive(Resource)]
...
impl FromWorld for GpuNormalsPipeline { fn from_world(world: &mut World) -> Self {
    let device = world.resource::<RenderDevice>();
    let layout = device.create_bind_group_layout(Some("metaballs.normals.layout"), &[
        // field texture (read-only storage)
        BindGroupLayoutEntry { binding:0, visibility: ShaderStages::COMPUTE, ty: BindingType::StorageTexture { access: StorageTextureAccess::ReadOnly, format: TextureFormat::Rgba16Float, view_dimension: TextureViewDimension::D2 }, count: None },
    ...
    let shader: Handle<Shader> = { let asset_server = world.resource::<AssetServer>(); asset_server.load("metaball://shaders/compute_3d_normals.wgsl") };
    #[cfg(any(not(feature = "shader_hot_reload"), target_arch = "wasm32"))]
    let shader: Handle<Shader> = embedded_shaders::normals_handle();

    let cache = world.resource::<PipelineCache>();
    let pipeline_id = cache.queue_compute_pipeline(ComputePipelineDescriptor { label: Some(Cow::Borrowed("metaballs.compute_normals")), layout: vec![layout.clone()], push_constant_ranges: vec![], shader, shader_defs: vec![], entry_point: Cow::Borrowed("compute_normals"), zero_initialize_workgroup_memory:false });
...
fn prepare_normals_bind_group(
    ...
    let Some(norm_img) = gpu_images.get(&normals.0) else { return; };
    let bind_group = render_device.create_bind_group(Some("metaballs.normals.bind_group"), &pipeline.layout, &[
        BindGroupEntry { binding:0, resource: BindingResource::TextureView(&field_img.texture_view) },
        BindGroupEntry { binding:1, resource: gpu_buffers.params.as_entire_binding() },
        BindGroupEntry { binding:2, resource: BindingResource::TextureView(&norm_img.texture_view) },
    ]);
    commands.insert_resource(GpuNormalsBindGroup(bind_group));
}

#[derive(Default)]
struct NormalsComputeNode { state: NodeState }
#[derive(Default)]
