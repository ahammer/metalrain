...
impl Plugin for MetaballShaderSourcePlugin {
    fn build(&self, app: &mut App) {
        #[cfg(all(feature = "shader_hot_reload", not(target_arch = "wasm32")))]
        {
            ...
            app.register_asset_source("metaball", AssetSourceBuilder::platform_default(&path_str, None));
        ...
        match ev {
            bevy::asset::AssetEvent::Modified { id } => {
                if let Some(path) = asset_server.get_path(*id) {
                    if path.path().to_string_lossy().contains("metaballs") || path.to_string().contains("metaball://shaders") {
