use crate::{Direction2D, EventEnvelope, EventPayload, GameEvent, InputEvent, PlayerAction};
use bevy::prelude::*;
...
impl FilterMiddleware {
    pub fn new(
        name: &'static str,
        pred: impl Fn(&EventEnvelope) -> bool + Send + Sync + 'static,
    ) -> Self {
        Self {
    ...
    pub fn with_default_gameplay() -> Self {
        ...
        km.map(KeyCode::KeyR, KeyMappingOutput::Game(GameEvent::ResetLevel))
            ...
            .map_many(
                ...
                KeyMappingOutput::Action(PlayerAction::Move(Direction2D::Up)),
            )
            .map_many(
                &[KeyCode::ArrowDown, KeyCode::KeyS],
                KeyMappingOutput::Action(PlayerAction::Move(Direction2D::Down)),
            )
            .map_many(
                &[KeyCode::ArrowLeft, KeyCode::KeyA],
                KeyMappingOutput::Action(PlayerAction::Move(Direction2D::Left)),
            )
            .map_many(
                &[KeyCode::ArrowRight, KeyCode::KeyD],
                KeyMappingOutput::Action(PlayerAction::Move(Direction2D::Right)),
            )
            .map(
                KeyCode::Space,
                KeyMappingOutput::Action(PlayerAction::PrimaryAction),
            );
        km
    }
    pub fn map(&mut self, key: KeyCode, out: KeyMappingOutput) -> &mut Self {
        self.mappings.insert(key, out);
        self
    }
    pub fn map_many(&mut self, keys: &[KeyCode], out: KeyMappingOutput) -> &mut Self {
        for k in keys {
            self.map(*k, out.clone());
        }
        self
    }
    pub fn with_mappings(&mut self, m: HashMap<KeyCode, KeyMappingOutput>) -> &mut Self {
        self.mappings = m;
        self
    }
}
    ...
    fn process(&mut self, ev: EventEnvelope) -> Option<EventEnvelope> {
        if let EventPayload::Input(InputEvent::KeyDown(code)) = &ev.payload {
            if let Some(out) = self.mappings.get(code).cloned() {
                let game_ev = match out {
                    KeyMappingOutput::Game(g) => g,
                    KeyMappingOutput::Action(a) => GameEvent::PlayerAction(a),
                };
                return Some(EventEnvelope {
                    payload: EventPayload::Game(game_ev),
                    ..ev
                });
            }
        }
...
impl Middleware for DebounceMiddleware {
    ...
    fn process(&mut self, ev: EventEnvelope) -> Option<EventEnvelope> {
        ...
        if let Some(&last) = self.last_seen.get(key) {
            if frame.saturating_sub(last) <= self.window_frames {
                return None;
            }
        }
...
impl CooldownMiddleware {
    pub fn new(cooldown_frames: u64) -> Self {
        Self {
...
impl Middleware for CooldownMiddleware {
    ...
    fn process(&mut self, ev: EventEnvelope) -> Option<EventEnvelope> {
        let key = event_kind_key(&ev);
        let frame = ev.frame_enqueued;
        if let Some(&last) = self.last_processed.get(key) {
            if frame < last + self.cooldown {
                return None;
            }
        }
