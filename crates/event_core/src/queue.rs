use crate::{EventEnvelope, EventResult, EventSourceTag, EventPayload, GameEvent};
use bevy::prelude::*;
...
impl EventQueue {
    pub fn with_capacity(cap: usize) -> Self {
        Self { incoming: VecDeque::new(), next_frame: VecDeque::new(), journal: VecDeque::new(), journal_capacity: cap }
    }
    ...
    pub fn drain_for_frame(&mut self, _frame: u64) -> Vec<EventEnvelope> {
        ...
        while let Some(ev) = self.incoming.pop_front() { drained.push(ev); }
        drained
    }
    pub fn promote_next_frame(&mut self) { while let Some(ev) = self.next_frame.pop_front() { self.incoming.push_back(ev); } }
    pub fn push_journal(&mut self, entry: JournalEntry) {
        if self.journal.len() == self.journal_capacity { self.journal.pop_front(); }
