...
fn custom_mapping_overrides_default() {
    ...
    km.map(KeyCode::KeyR, KeyMappingOutput::Game(GameEvent::PauseGame))
      .map(KeyCode::Space, KeyMappingOutput::Game(GameEvent::ResetLevel));
    ...
    app.world_mut().resource_mut::<EventQueue>().enqueue(EventEnvelope::new(EventPayload::Input(InputEvent::KeyDown(KeyCode::KeyR)), EventSourceTag::Input, frame), frame);
    app.update();
