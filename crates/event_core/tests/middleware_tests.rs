...
fn key_mapping_converts_input() {
    ...
    assert!(app
        .world()
        .get_resource::<handlers::TargetCounter>()
        .is_some());
...
fn filter_short_circuits() {
    ...
    app.register_middleware(FilterMiddleware::new("NoPause", |env| {
        !matches!(env.payload, EventPayload::Game(GameEvent::PauseGame))
    }));
    // enqueue P
    let frame = app.world().resource::<FrameCounter>().0;
    app.world_mut().resource_mut::<EventQueue>().enqueue(
        EventEnvelope::new(
            EventPayload::Input(InputEvent::KeyDown(KeyCode::KeyP)),
            EventSourceTag::Input,
            frame,
        ),
        frame,
