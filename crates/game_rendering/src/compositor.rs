...
pub struct LayerBlendState {
    pub blend_modes: [BlendMode; 3],
}

impl Default for LayerBlendState {
    fn default() -> Self {
        Self {
            blend_modes: [BlendMode::Normal, BlendMode::Normal, BlendMode::Additive],
        }
    }
}
...
pub struct CompositorUniforms {
    pub settings: Vec4,
    pub enabled_low: Vec4,       // x,y,z used (background, game, metaballs)
    pub blend_modes_low: Vec4,   // x,y,z used; w unused padding
}
    ...
    fn default() -> Self {
        Self {
            uniforms: CompositorUniforms {
                ...
                enabled_low: Vec4::new(1.0, 1.0, 1.0, 0.0),
...
fn update_uniforms(
    ...
    for (idx, layer) in RenderLayer::ALL.iter().enumerate() {
        ...
        enabled_low[idx] = if enabled { 1.0 } else { 0.0 };
        blend_low[idx] = encode_blend_mode(blend_mode) as f32;
    }
