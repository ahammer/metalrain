...
fn track_cursor(
    ...
    if let Some(pos) = window.cursor_position() {
        if let Ok(world) = camera.viewport_to_world_2d(cam_transform, pos) {
...
fn update_overlay(
    ...
    let cursor_line = if let Some(p) = cursor.world {
        format!("Cursor: ({:.0},{:.0})", p.x, p.y)
    } else {
        "Cursor: --".to_string()
    };
    let last_key = stats
 ...
 Left Click: register click\n Move mouse: update cursor\n Press keys: update stats",
        ...
        if stats.clicks_total == 0 {
            0
        } else {
            stats.clicks_dropped * 100 / stats.clicks_total
        },
        stats.keys_total,
        last_key,
    );
