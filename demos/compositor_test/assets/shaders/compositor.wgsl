...
fn layer_enabled(index: u32) -> f32 {
    ...
    switch (index) {
        case 0u: {
            value = compositor.enabled_low.x;
        }
        case 1u: {
            value = compositor.enabled_low.y;
        }
        case 2u: {
            value = compositor.enabled_low.z;
        }
        case 3u: {
            value = compositor.enabled_low.w;
        }
        case 4u: {
            value = compositor.enabled_high.x;
        }
        default: {}
    }
    return value;
...
fn layer_blend(index: u32) -> u32 {
    ...
    switch (index) {
        case 0u: {
            value = compositor.blend_modes_low.x;
        }
        case 1u: {
            value = compositor.blend_modes_low.y;
        }
        case 2u: {
            value = compositor.blend_modes_low.z;
        }
        case 3u: {
            value = compositor.blend_modes_low.w;
        }
        case 4u: {
            value = compositor.blend_modes_high.x;
        }
        default: {}
    }
...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if layer_enabled(0u) > 0.5 {
        color = sample_layer(background_tex, background_sampler, uv);
    }

    if layer_enabled(1u) > 0.5 {
        let layer = sample_layer(game_tex, game_sampler, uv);
        color = apply_blend(color, layer, layer_blend(1u));
    }

    if layer_enabled(2u) > 0.5 {
        let layer = sample_layer(metaballs_tex, metaballs_sampler, uv);
        color = apply_blend(color, layer, layer_blend(2u));
    }

    if layer_enabled(3u) > 0.5 {
        let layer = sample_layer(effects_tex, effects_sampler, uv);
        color = apply_blend(color, layer, layer_blend(3u));
    }

    if layer_enabled(4u) > 0.5 {
        let layer = sample_layer(ui_tex, ui_sampler, uv);
        color = apply_blend(color, layer, layer_blend(4u));
    }
